...
export default function Home() {
  ...
  function handleKeyPressed(e){
    console.log('heloo', e)
    if(e.charCode === 13){
            fetchMovieInfo(); // Ensure it is only this code that runs
  ...
  const fetchMovieInfo =() => {
    ...
    let new_array = m_movie.filter((movie) =>  {
          if(MovieName === "" )
          {
            return "";
          }
  ...
  return (
    ...
    <div  className = {styles.SEARCHBAR}>
      ...
      <input type={'text'} value={search} className='Search_Bar' onKeyPress={handleKeyPressed} placeholder='Search a Movie ' 
      onChange={(e)=> setText(e.target.value)}

      />
      {/* <button className='Buttonsearch' disabled={isEmpty} onKeyPress={handleKeyPressed} >SEARCH</button>  */}
